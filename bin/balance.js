...
//
// Balance is a property of the MECHANICS, not of the words — every world runs the
// same event table. So this sweeps seeds across all worlds and reports one spread.
//
...
for (let seed = 1; seed <= N; seed++) {
  const e = new Engine({ seed, dials, lore: packs[seed % packs.length] });
  e.run(days);
  const s = e.state;
  runs.push({
    alive: s.alive,
...
console.log(`\n${N} seeds x ${days} days across ${packs.length} worlds   dials: reckless ${dials.reckless}  sociable ${dials.sociable}  generous ${dials.generous}`);
...
line('attention', num((x) => x.attention));
...
console.log(`  drew the power's eye (>=10) ${pct((x) => x.attention >= 10)}%`);
console.log('');
